...
function App() {
  ...
  return (
    <div className="shell">
      ...
      <section className="panel"><p className="panel-label">Recommended next scope</p><h2>What I’d add next</h2><ul className="nice-list">{scopeRecommendations.map((item) => <li key={item}>{item}</li>)}</ul></section>

      <section className="panel"><p className="panel-label">House profile</p><h2>Assumptions used</h2><ul className="nice-list"><li>{houseProfile.sizeSqFt} sq ft, {houseProfile.levels} levels, {houseProfile.bedrooms} bedrooms, {houseProfile.bathrooms} baths.</li><li>{houseProfile.hvac.system} with {houseProfile.hvac.heads} indoor heads.</li><li>Shared-household ready Firestore structure instead of a throwaway local-only schema.</li><li>Shopping built in from the start for Home Depot, grocery, and Costco runs.</li></ul></section>
    </div>
  )
}
