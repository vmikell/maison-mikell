...
function buildChecks() {
  ...
  checks.push({ ok: !nativeAuthModeHint.value || nativeAuthModeHint.value === 'native-bridge', level: 'warn', label: 'Native bridge auth mode hint', detail: nativeAuthModeHint.value ? `VITE_NATIVE_GOOGLE_AUTH_MODE=${nativeAuthModeHint.value} (${nativeAuthModeHint.source})` : 'No VITE_NATIVE_GOOGLE_AUTH_MODE override found. Native shells default to native-bridge; set firebase-web-redirect only for fallback testing.' })
